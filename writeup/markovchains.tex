...
Then, we will will deduce the relevant theorems and develop a whole swath of properties for square regular transition matrices and stationary vectors .
...
Let $A$ be a matrix in $\mathbb{C}$ such that the characteristic polynomial of $A$ splits over $\mathbb{C}$ and thus $A \sim J = \oplus_{i=1}^rJ_{n_i}(\lambda_i) \in M_{n \times n}\mathbb{C}$, where $\lambda_1, \lambda_2, \ldots \lambda_r$ are the distinct eigenvalues of $A$ and $J_{n_i}(\lambda_i)$ is the $n_i \times n_i$ Jordan block corresponding to eigenvalue $\lambda_i$.
		
		
Then by $lim_{m \to \infty}A^m$ exists if and only if $lim_{m \to \infty}J^m$ exists.
...
\todo{sizeofjordanblcokgreaterthan2?}
Let $J|\lambda|$ be the Jordan block corresponding to eigenvalue $\lambda$ of some matrix $A \in M_{n \times n}(\mathbb{C})$ with $n \geq 2$.
Then  $\forall \ell \in \mathbb{N}_{> 0}$ we have 
$$(J_{\lambda = 1}^{\ell})_{12} = \ell \lambda^{\ell -1}$$.
...
\begin{enumerate}
	...
	\item 
		...
		But we know by lemma \ref{jordanblockexp} for $n \geq 2$ that $(J_{\lambda = 1}^{\ell})_{12} = \ell \lambda^{\ell -1} = \ell * 1 = \ell$.
		But $(J_{1}^{\ell})_{12}$ is an element of $J^m$ and thus $||J^m||$ is not bounded, a contradiction!
...
\begin{enumerate}
	...
	\item
			...
			%Finally, we know $E_{\lambda} \subseteq K_{\lambda}$ by Theorem 7.1.
	...
	\end{comment}
		Since $A$ is a matrix in $\mathbb{C}$, we know the characteristic polynomial of $A$ splits over $\mathbb{C}$ and thus $A \sim J = \oplus_{i=1}^rJ_{n_i}(\lambda_i) \in M_{n \times n}\mathbb{C}$, where $\lambda_1, \lambda_2, \ldots \lambda_r$ are the distinct eigenvalues of $A$.
		
		
		Then by lemma \ref{benstheorem1}, we know $lim_{m \to \infty}A^m$ exists if and only if $lim_{m \to \infty}J^m$ exists.
		Consider $\lambda_i\ \forall\ 1 \leq i \leq r $, we know by \ref{theorem519} that either $|\lambda_i| < 1$ or $\lambda_i = 1$.
		Thus

		$$lim_{m \to \infty}J^m = lim_{m \to \infty}(\oplus_{i=1}^rJ_{n_i}(\lambda_i))^m = \oplus_{i=1}^rlim_{m \to \infty}(J_{n_i}(\lambda_i))^m$$

		$$ = \oplus_{i=1 \lambda = 1}^rlim_{m \to \infty}(J_{n_i}(\lambda_i))^m \oplus\ \oplus_{i=1 |\lambda| < 1}^rlim_{m \to \infty}(J_{n_i}(\lambda_i))^m$$

		%$$lim_{m \to \infty}((\oplus_{i=1, \lambda_i = 1}^rJ_{n_i}(1))^m \oplus  \oplus_{i=1, |\lambda_i| < 1}^r(J_{n_i}(\lambda_i))^m $$

		But by lemma \ref{exericse7221}, we know the size of the blocks corresponding to $\lambda = 1$ have size $1 \times 1$.
		Thus $\forall i$ such that $\lambda_i =1$, we know $n_i = 1$ and moreover $J_1(1) = 1$.
		Thus $lim_{m \to \infty}(J_{1}(1))^m = lim_{m \to \infty} (1)^m = 1 $ and thus $\oplus_{i=1 \lambda = 1}^rlim_{m \to \infty}(J_{n_i}(\lambda_i))^m $ converges.


		Additionally, for $|\lambda_i| < 1$, $J_{n_i}(\lambda_i)$ is an upper triangular matrix with values $|\lambda_i| < 1$ along the diagonal.
		Thus $\forall i$ such that $|\lambda_i| < 1$  we have that $lim_{m \to \infty}(J_{n_i}(\lambda_i))^m$ converges to $0_{n_i}$.

		Thus
		$$lim_{m \to \infty}J^m = \oplus_{i=1 \lambda = 1}^rlim_{m \to \infty}(J_{n_i}(\lambda_i))^m \oplus \oplus_{i=1 |\lambda| < 1}^r0_{n_i} = \oplus_{i=1 \lambda = 1}^rlim_{m \to \infty}(J_{n_i}(\lambda_i))^m$$

		And thus $lim_{m \to \infty}J^m$ exists and we can conclude $lim_{m \to \infty}A^m$ exists.
