...
\begin{enumerate}
	...
	\end{comment}
		...
		Then by \todo{ben's theorem}, we know $lim_{m \to \infty}A^m$ exists if and only if $lim_{m \to \infty}J^m$ exists.
		...
		Thus $\forall i$ such that $|\lambda_i| < 1$  we have that $lim{m \to \infty}(J_{n_i}(\lambda_i))^m$ converges to 0.
		...
		And thus $lim_{m \to \infty}J^m$ exists and thus $lim_{m \to \infty}A^m$ exists.
	...
	\item 
			...
			\todo{tighten up argument}
			\todo{if x is a probability vector then it's the only one in E because it has to sum to 1, SHOW!}
			We know $AL = L$ by $(4)$.
