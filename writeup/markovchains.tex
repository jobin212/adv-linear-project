...
Let $A$ is a matrix in $\mathbb{C}$ such that the characteristic polynomial of $A$ splits over $\mathbb{C}$ and thus $A \sim J = \oplus_{i=1}^rJ_{n_i}(\lambda_i) \in M_{n \times n}\mathbb{C}$, where $\lambda_1, \lambda_2, \ldots \lambda_r$ are the distinct eigenvalues of $A$ and $J_{n_i}(\lambda_i)$ is the $n_i \times n_i$ Jordan block corresponding to eigenvalue $\lambda_i$.
...
Let $J_|\lambda|$ be the Jordan block corresponding to eigenvalue $\lambda$ of some matrix $A \in M_{n \times n}\mathbb{C}$ with $n \geq 2$.
